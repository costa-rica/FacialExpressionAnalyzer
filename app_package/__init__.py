...
logger_init.info(f'--- Starting Facial Expression Analyzer ---')
...
logger_init.info(f"- fea_config_TYPE: {os.environ.get('fea_config_TYPE')}")
...
def create_app(config_for_flask = config):
    ...
    create_folder(config_for_flask.DIR_USER_IMAGES)
    ############################################################################
